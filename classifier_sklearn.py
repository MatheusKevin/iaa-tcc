...
def print_confusion_matrix(confusion_matrix, df_classes, figsize=(10, 7), fontsize=14):
    class_names = df_classes['class_descr'].unique()
    df_cm = pd.DataFrame(
...
def plot_class_chart_bar(train_vals, test_vals, df_classes):
    ...
    plt.xticks(x, df_classes['class_descr'].unique(), rotation=45, size=15)
...
def generate_svm_classifier(label_file, df_classes):
    ...
    rfc = RandomForestClassifier(n_estimators=100, random_state=11, verbose=2)
    ...
    print_base_info(y_train, y_test, df_classes.filter(items=np.unique(y_train), axis=0))
    ...
    print_heatmap(y_test, y_pred, df_classes.filter(items=np.unique(y_test), axis=0))
...
def predict_validation_videos(clf_model):
    ...
    for index, dict in df_dict.iterrows():
        ...
        for file in tqdm(file_names):
            ...
            y_valid.append(dict['class_group_id'])
            y_pred.append(determine_pred_class(y_frames_pred))

    evaluate_classifier(y_valid, y_pred)
    # print_heatmap(y_valid, y_pred, class_names_all)


file_name = 'labels_group'
# clf = generate_svm_classifier(file_name, class_names_all)
# f = open(os.path.join(MODEL_DIR, 'clf_' + file_name + '.pkl'), 'wb')
# pickle.dump(clf, f)
f = open(os.path.join(MODEL_DIR, 'clf_' + file_name + '.pkl'), 'rb')
clf = pickle.load(f)
